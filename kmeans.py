...
def selectedFeaturesModelV2(data, bestModelName, pipeDict, pipeline):
    ...
    finalLabels = finalModelObject.labels_
...
def kmeansDataStack(label1, label2, label3, label4):
    labelList = [label1, label2, label3, label4]
    df = pd.concat([pd.Series(x) for x in labelList], axis=1)
    df.to_csv('kmeansDataStack.csv', index=False)
...
if __name__ == '__main__':
    ...
    clusterModel = KMeans(n_clusters=kCluster, init='k-means++')
    ...
    kmeansDataStack(labels, pcaLabels, fsLabels, labelsV2)
    print('kmeansDataStackReady!..')